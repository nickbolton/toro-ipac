...
System.out.println("UNABLE TO connection to url");
    ...
    public static String securePost(String xml, String urlLocation) 
    throws IOException {
    ...
    public static String post(String xml, String urlLocation) 
    throws IOException {
        URL url = getURL(urlLocation);
        if (url == null) return null;
...
System.out.println("SERVER : " + httpServer);
        if (httpServer.startsWith("https://"))
            responseMsg = securePost(data(), httpServer);
        else if (httpServer.startsWith("http://"))
            responseMsg = post(data(), httpServer);
        else
            System.out.println(
                "Server location need to started with http or https");
